...
class Generic_WSI_Classification_Dataset(Dataset):
	...
	def return_splits(self, from_id=True, csv_path=None):
		...
		if from_id:
			...
			train_split = self.get_split_from_df(all_splits, 'train')  # feature, label
		...
		if not self.use_h5:
			if self.data_dir_s and self.data_dir_l:
				if(self.mode == 'transformer'):
					full_path_s = os.path.join(data_dir_s, 'pt_files', '{}.pt'.format(slide_id))
					features_s = torch.load(full_path_s)
					full_path_l = os.path.join(data_dir_l, 'pt_files', '{}.pt'.format(slide_id))
					features_l = torch.load(full_path_l)
					#attn_path = os.path.join(data_dir_l, 'attn_mask', '{}.pt'.format(slide_id.rstrip('.svs')))
					#attn_mask = torch.load(attn_path)
					#coord_path_s = os.path.join(data_dir_s, 'h5_files', '{}.h5'.format(slide_id))
					#coords_s = torch.from_numpy(np.array(h5py.File(coord_path_s, 'r')['coords']))
					#coord_path_l = os.path.join(data_dir_l, 'h5_files', '{}.h5'.format(slide_id))
					#coords_l = torch.from_numpy(np.array(h5py.File(coord_path_l, 'r')['coords']))
