...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_auc, model, ckpt_name):
        if self.verbose:
            print(f'Validation auc increased ({self.val_auc:.6f} --> {val_auc:.6f}).  Saving model ...')
...
def train(datasets, cur, args):
    ...
    # 调用 KMG-Mamba，模型类型和 MG_Trans 一致 （多倍率）
    if args.model_type == 'MG_Trans':
        from models.model_MG_Mamba import MG_Mamba_main, CONFIGS
        ...
        model_dict = {'config': config, 'img_size': args.img_size, 'num_classes':args.n_classes, 'sequence_length_min':args.sequence_length_min, 'zero_head':True, 'smoothing_value':args.smoothing_value}
        model = MG_Mamba_main(**model_dict)
    ...
    _, val_error, val_auc, _, val_f1, val_acc = summary(args.mode, model, val_loader, args.n_classes)
    print('Val error: {:.4f}, ROC AUC: {:.4f}, F1: {:.4f}, ACC: {:.4f}'.format(val_error, val_auc, val_f1, val_acc))

    results_dict, test_error, test_auc, acc_logger, test_f1, test_acc = summary(args.mode, model, test_loader, args.n_classes)
    print('Test error: {:.4f}, ROC AUC: {:.4f}, F1: {:.4f}, ACC: {:.4f}'.format(test_error, test_auc, test_f1, test_acc))
...
def test(datasets, cur, args, ckpt_path):
    """   
        test for a single fold
    """
    print('\nTesting Fold {}!'.format(cur))
    #model = initiate_model(args, ckpt_path)
    model_dict = {'n_classes': args.n_classes}
    
    if args.model_type == 'MG_Trans':
        from models.model_MG_Mamba import MG_Mamba_main, CONFIGS
        config = CONFIGS['MG_Trans']
        config.select_ratio = 0.1
        config.transformer.num_layers = 12
        config.transformer.num_heads = 3
        model_dict = {'config': config, 'img_size': 3000, 'num_classes': 2, 'sequence_length_min':0, 'zero_head':True, 'smoothing_value':0.0}
        model = MG_Mamba_main(**model_dict)
    ...
    return results_dict, test_auc, 1-test_error, each_class_acc, test_f1
...
def validate(cur, epoch, model, loader, n_classes, early_stopping = None, writer = None, loss_fn = None, results_dir=None):
    ...
    with torch.no_grad():
        for batch_idx, (data_s, data_l, label) in enumerate(loader):
            data_s, data_l, label = data_s.to(device, non_blocking=True),  \
            ...
            all_pred.append(Y_hat.cpu()[0][0])
            all_label.append(label.cpu()[0])
...
def summary(mode, model, loader, n_classes):
    ...
    return patient_results, test_error, auc, acc_logger, test_f1, test_acc
