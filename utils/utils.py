...
def collate_tranformer(batch):
	...
	#attn_mask = torch.cat([item[5] for item in batch], dim = 0)
...
def collate_graph(batch):
	...
	attn_mask = torch.cat([item[5] for item in batch], dim = 0)
	return [img_s, coord_s, img_l, coord_l, label, attn_mask]
...
def generate_split(cls_ids, val_num, test_num, samples, n_splits = 5,
	...
	for i in range(n_splits):
		...
		for c in range(len(val_num)):
			possible_indices = np.intersect1d(cls_ids[c], indices) #all indices of this class，找到两个数组的交集
			val_ids = np.random.choice(possible_indices, val_num[c], replace = False) # validation ids

			remaining_ids = np.setdiff1d(possible_indices, val_ids) #indices of this class left after validation，在possible_indices中，不在val_ids中
