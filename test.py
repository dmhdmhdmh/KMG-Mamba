...
parser.add_argument('--data_root_dir', type=str, default='/home/mhduan/projectsummer/data/CAMELYON16-Feat-PLIP',
                    help='data directory')
...
parser.add_argument('--save_exp_code', type=str, default='None_s2024_k9',
                    help='experiment code to save eval results')
parser.add_argument('--models_exp_code', type=str, default='None_s2024_k9',
...
parser.add_argument('--split_dir', type=str, default='/home/mhduan/projectsummer/MIL/miccaicheck/CAMELYON/plip/MG-Mamba-Cross-Concat-Wikg-Ins/splits/Yifuyuan/task_1_tumor_vs_normal_100',
...
parser.add_argument("--sequence_length_min", default=0, type=int,
                    help="c16:0, nsclc:0, brca:8, rcc:0")
...
if args.task == 'task_1_tumor_vs_normal':
    args.n_classes=2
    dataset = Generic_MIL_Dataset(csv_path = 'dataset_csv/CAMELYON_label.csv',
                                  mode = args.mode,
...
elif args.task == 'task_subtyping_nsclc':
    args.n_classes=2
    dataset = Generic_MIL_Dataset(csv_path = 'dataset_csv/NSCLC_label.csv',
                                  mode = args.mode,
                                  data_dir_s = os.path.join(args.data_root_dir, args.data_folder_s),
                                  data_dir_l = os.path.join(args.data_root_dir, args.data_folder_l),
                                  shuffle = False,
    ...
    args.n_classes=2
    dataset = Generic_MIL_Dataset(csv_path = 'dataset_csv/BRCA_label.csv',
                                  mode = args.mode,
                                  data_dir_s = os.path.join(args.data_root_dir, args.data_folder_s),
                                  data_dir_l = os.path.join(args.data_root_dir, args.data_folder_l),
                                  shuffle = False,
...
def main(args):
    ...
    for i in folds:
        ...
        results, test_auc, test_acc, each_class_acc, test_f1 = test(datasets, i, args, ckpt_paths[i])
    ...
    print("")
    print(f'test_auc average : {np.mean(all_test_auc):.6f}, std : {np.std(all_test_auc):.6f}')
    print(f'test_acc average : {np.mean(all_test_acc):.6f}, std : {np.std(all_test_acc):.6f}')
