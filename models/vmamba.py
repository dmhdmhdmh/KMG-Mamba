...
if True:
    ...
    class CrossScan_multimodal(torch.autograd.Function):
        ...
        def forward(ctx, x_rgb: torch.Tensor, x_e: torch.Tensor):
            # B, C, H, W -> B, 2, C, 2 * H * W
            B, C, H, W = x_rgb.shape
            B_, C_, H_, W_ = x_e.shape
            ctx.shape = (B, C, H, W, H_, W_)
            xs_fuse = x_rgb.new_empty((B, 2, C, H_ * W_ + H * W))
        ...
        def backward(ctx, ys: torch.Tensor):
            # out: (b, 2, d, l)
            B, C, H, W, H_, W_ = ctx.shape
            L = H * W + H_ * W_
            ys = ys[:, 0] + ys[:, 1].flip(dims=[-1]) # B, d, 2 * H * W
            # get B, d, H*W
            return ys[:, :, 0:H*W].view(B, -1, H, W), ys[:, :, H*W:H*W+H_*W_].view(B, -1, H_, W_) 
         
    class CrossMerge_multimodal(torch.autograd.Function):
        @staticmethod
        def forward(ctx, ys: torch.Tensor, L1, L2):
            B, K, D, L = ys.shape
            # ctx.shape = (H, W)
            # ys = ys.view(B, K, D, -1)
            ys = ys[:, 0] + ys[:, 1].flip(dims=[-1]) # B, d, 2 * H * W, broadcast
            # y = ys[:, :, 0:L//2] + ys[:, :, L//2:L]
            return ys[:, :, 0:L1], ys[:, :, L1:L]
    ...
    def cross_selective_scan_multimodal_k2(
        ...
        ys: torch.Tensor = selective_scan(
            x_fuse, dts, As, Bs, Cs, Ds, delta_bias, delta_softplus, nrows,
        ).view(B, K, -1, H*W + H_*W_)
        
        y_rgb, y_e = CrossMerge_multimodal.apply(ys, H*W, H_*W_)

        y_rgb = y_rgb.transpose(dim0=1, dim1=2).contiguous().view(B, H, W, -1)
        y_e = y_e.transpose(dim0=1, dim1=2).contiguous().view(B, H_, W_, -1)
...
class ConMB_SS2D(nn.Module):
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        ...
        y_e = self.dropout(self.out_proj(y_e))
        return y_rgb, y_e
...
# Cross-scale Selective Scan Module 
class Cross_Mamba_Attention_SSM(nn.Module):
    def __init__(
        ...
        self.normc = nn.LayerNorm(self.d_state)
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        selective_scan = selective_scan_fn_v1
        B, L, d = x_rgb.shape
        x_rgb = x_rgb.permute(0, 2, 1) # Low scale features
        B_, L_, d_ = x_e.shape
        x_e = x_e.permute(0, 2, 1) # High scale features
        ...
        C_rgb_shortcut = C_rgb
        C_e_shortcut = C_e
        Transpose_matrix = torch.matmul(C_rgb, C_e.T) 
        C_rgb = C_e_shortcut + self.normc(torch.matmul(Transpose_matrix.T, C_rgb)) # Information Exchange
        C_e = C_rgb_shortcut + self.normc(torch.matmul(Transpose_matrix, C_e)) # Information Exchange
        
        dt_rgb = self.dt_proj_1.weight @ dt_rgb.t()
        dt_e = self.dt_proj_2.weight @ dt_e.t()
        dt_rgb = rearrange(dt_rgb, "d (b l) -> b d l", l=L)
        dt_e = rearrange(dt_e, "d (b l) -> b d l", l=L_)
        ...
        B_e = rearrange(B_e, "(b l) dstate -> b dstate l", l=L_).contiguous()
        C_rgb = rearrange(C_rgb, "(b l) dstate -> b dstate l", l=L_).contiguous()
...
class CrossMambaFusion_SS2D_SSM(nn.Module):
    '''
    Cross-scale Knowledge Interaction Mamba
    '''
    def __init__(
        self,
        # basic dims ===========
        ...
        **kwargs,
        ...
        self.norm_rgb = nn.LayerNorm(self.d_inner)
        self.norm_e = nn.LayerNorm(self.d_inner)
        # Cross-scale Selective Scan Module 
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        ...
        if self.d_conv > 1:
            ...
            y_rgb, y_e = self.CMA_ssm(x_rgb, x_e) # Cross-scale selective scanning
            y_rgb = x_rgb + y_rgb
            y_e = x_e + y_e

        out_rgb = self.out_proj_rgb(self.norm_rgb(y_rgb))
        out_e = self.out_proj_e(self.norm_e(y_e))
...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input: torch.Tensor):
        ...
        x = self.drop_path(self.norm3(x)) + x_shortcut
        return x
...
class CrossMambaFusionBlock(nn.Module):
    ...
    def _forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        x_rgb_cross, x_e_cross = self.op(x_rgb, x_e) # x_rgb represents low scale features, x_e represents high scale features
