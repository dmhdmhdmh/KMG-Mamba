...
from models.mamba.mamba_ssm.ops.selective_scan_interface import selective_scan_fn, mamba_inner_fn, mamba_inner_fn_no_out_proj
...
class SRMamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None, rate=10):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            ...
            xz_b_t = TransposeTokenReEmbedding.transpose_normal_padding(xz_b, rate=rate)
            out_b_t = mamba_inner_fn_no_out_proj(
                    xz_b_t,
            ...
            # 将序列转置，并删除padding的位置
            out_b_t = TransposeTokenReEmbedding.transpose_remove_padding(out_b_t, rate=rate, length=L)
            out_b_t = out_b_t.flip([-1])

            xz_t = TransposeTokenReEmbedding.transpose_normal_padding(xz, rate=rate)
            out_t = mamba_inner_fn_no_out_proj(
                    xz_t,
                    self.conv1d_b.weight,
                    self.conv1d_b.bias,
                    self.x_proj_b.weight,
            ...
            # 将序列转置，并删除padding的位置
            out_t = TransposeTokenReEmbedding.transpose_remove_padding(out_t, rate=rate, length=L)
            out = F.linear(rearrange(out + out_t, "b d l -> b l d"), self.out_proj.weight, self.out_proj.bias)
            out_b = F.linear(rearrange(out_b + out_b_t, "b d l -> b l d"), self.out_proj.weight, self.out_proj.bias)
            out = out + out_b
        else:
            # x, z拆分用于两个分支的聚合
